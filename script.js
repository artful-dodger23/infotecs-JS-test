...
//функции для фильтрации по алфавиту имени и фамилии
...
function SortByFirstNameAscending(){
  Data.sort((a, b) => b.name.firstName.localeCompare(a.name.firstName));
  clearTable();
  createTable(Data)
}
...
function SortByLastNameAscending(){
  Data.sort((a, b) => b.name.lastName.localeCompare(a.name.lastName));
  clearTable();
  createTable(Data)
}
//функция очистки таблицы при фильтрации и пересоздании таблицы
function clearTable()
{
  var tableToClear = document.getElementById("target-table");
  tableToClear.innerHTML = ("");
}
//функция ссздания таблицы из объектов JSON
function createTable(items)
{
  clearTable();
  target = document.getElementById("target-table");
  titleRow = document.createElement('tr');
  titleRow.innerHTML = `<td  class="firstRow">First Name</td><td  class="firstRow">Last Name</td><td class="firstRow">Eye Color</td><td  class="firstRow">About</td>`;

  target.append(titleRow);

  items.forEach(item => {
    tableRow = document.createElement('tr');
    tableRow.innerHTML = `<tr><td class="id-row">${item.id}</td><td>${item.name.firstName}</td><td  class="highlighted">${item.name.lastName}</td><td >${item.eyeColor}</td><td class="highlighted" class="col4">${item.about}</td>
    <td><button onclick="createForm('${item.id}')">Edit</button></td></tr>`;
    target.append(tableRow);
  });

}

createTable(Data); //вызов таблицы при первой загрузке страницы
...
target = document.getElementById("editing-form")

let item = Data.find(x => x.id == id );
console.log(item)

target.innerHTML+= `<h2>Edit Information</h2>`
target.innerHTML += `<input id="id" type="hidden" value="${item.id}"></input>`;

  target.innerHTML += `<input id="firstName" type="text" placeholder="${item.name.firstName}" value="${item.name.firstName}"></input><br>`;
  target.innerHTML += `<input id="lastName" type="text" placeholder="${item.name.lastName}" value="${item.name.lastName}"></input><br>`;
  target.innerHTML += `<input id="eyeColor" type="text" placeholder="${item.eyeColor}" value="${item.eyeColor}"></input><br>`;
  target.innerHTML += `<textarea id="about" form="editing-form" >${item.about}</textarea><br>`;
  target.innerHTML += `<button type="submit">Submit</button>`;
  target.innerHTML += `<button onclick=clearEditForm()>Cancel</button>`;

  form = document.getElementById("editing-form").addEventListener("submit", function(e) {
      e.preventDefault(); 
...
//обработка данных при редактировании ряда
function processForm() {

  var form = document.getElementById("editing-form");

  let item = Data.find(x => x.id == form.elements["id"].value);

  item.name.firstName = form.elements['firstName'].value; 

  item.name.lastName = form.elements['lastName'].value; 
  
  item.eyeColor = form.elements['eyeColor'].value; 

  item.about = form.elements['about'].value; 
...
//очистка формы при отмене редактирования
function clearEditForm(){
  var formToClear = document.getElementById("editing-form");
  formToClear.innerHTML = ("");
};
